...
export function checkCollision(newTimeStr, container, draggedElementId = null) {
    ...
    for (let i = 0; i < dayElements.length; i++) {
        ...
        // ИЗМЕНЕНО: Теперь все элементы (занятия и перерывы) должны иметь dataset.startTime и dataset.endTime
        ...
        if (newStartMin < exEnd && newEndMin > exStart) {
            // Если это блок перерыва и автоматические перерывы отключены, игнорируем коллизию с ним
            if (element.classList.contains('break-block') && !breakToggleEnabled) {
                continue; 
            }
            return true;
        }
