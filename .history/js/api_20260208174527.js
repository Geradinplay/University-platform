...
export async function getSchedule() {
    try {
        const response = await fetch(`${API_BASE_URL}/api/schedule`); // Здесь /api не нужен
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        return await response.json();
...
export async function updateLessonDay(lessonId, newDay) {
    try {
        ...
        // Если сервер возвращает обновленный объект, можно его обработать.
        // В данном случае просто убеждаемся, что запрос прошел успешно.
        return true; 
    } catch (error) {
