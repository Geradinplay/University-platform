...
export function addNewLesson() {
    ...
    const selectedSubject = subjectSelect.options[subjectSelect.selectedIndex].textContent; // Получаем выбранный предмет
    ...
    if (subjectSelect.value === "") { // Добавлена валидация для предмета
        alert("Пожалуйста, выберите предмет.");
        return;
    }
    // Удален if (!t) - название теперь берется из subjectSelect
