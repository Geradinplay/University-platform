...
// ИЗМЕНЕНО: Используем updateLessonDay из api.js
...
export async function drop(ev) {
    ...
    let newDay;
    // ИЗМЕНЕНО: Целевой контейнер для буфера
    let targetContainer = ev.target.closest('.day') || ev.target.closest('#buffer-container'); 
    ...
    if (targetContainer.id === 'buffer-container') { // ИЗМЕНЕНО: ID буфера
        newDay = 0; // Для буфера
        document.getElementById('buffer-cards-container').appendChild(el); // Добавляем в контейнер для карточек буфера
